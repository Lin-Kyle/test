...
[Javascript執行機制--單線程，同異步任務，事件循環](https://www.qdfuns.com/article/40831/9bcef0e2d1dc88f67eb44e4517d494b2.html)
...
简单总结:
**阻塞I/O和非阻塞I/O区别在于：在I/O操作的完成或数据的返回前是等待還是返回！(可以理解成一直等還是分時間段等)**
**同步I/O和异步I/O区别在于    ：在I/O操作的完成或数据的返回前会不会将进程阻塞(或者說是主動查詢還是被動等待通知)！**

用个生活化的例子就是等外卖吧
阻塞I/O: 白领A下完单就守著前臺小姐直到收到外卖才离开,后面其他人在排队等他走开;
非阻塞I/O: 白领B下完单每隔一段时间就去询问前臺小姐外卖到了没,然后回去上班,需要来回走多次並且也要排隊但是妨礙其他人的時間較少;
...
所谓的事件驱动就是nodejs里有个事件队列,每个进来的请求處理完就被關閉然後繼續服務下一個請求,当这个请求完成会被推进处理队列,然後通过一种循环方式检测队列事件有没变化,有就执行相对应的回调函数,没有就跳过到下一步,如此往復.
...
当nodejs开始运行的时候会初始化事件循环,处理所提供的输入脚本或者放置进REPL[color=#b1b1b1]\(Read Eval Print Loop:交互式解释器类似 Window 系统的终端或 Unix/Linux shell)[/color],可能会进行异步API调用.定时器调度,或者process.nextTick(),然后开始处理事件循环的流程.
...
* poll(轮询): 检索新的I/O事件;执行I/O相关回调(几乎是关闭回调,定时器调度,和setImmediate()),当运行时候適當條件下nodejs会佔用阻塞;
...
这阶段有两个主要功能:
1, 计算I/O应该阻塞和轮询的时间;
(原文: Calculating how long it should block and poll for I/O, then,我看到有些人會翻譯成当 timers 的定时器到期后，执行定时器（setTimeout 和 setInterval）的 callback。不知道版本不對還是我翻譯不對味)
2, 处理poll队列事件;
...
| delay| 必需。在执行代码前需等待的毫秒数, W3C標准規定時間間隔低于4ms被算爲4ms，具體看浏覽器|
...
總的來說單線程的鍋.
1, 异常抛出終止
我們都知道Javascript是一門單線程語言,在发生各种错误之後,JavaScript引擎通常会停止，并抛出一个错误.
Nodejs具體錯誤直接看[Error (错误)](http://nodejs.cn/api/errors.html).
暫時還沒研究到,但是肯定可以通過一些方法解決的,後補.

2, 不適合CPU密集型
儘管我們上面已經提出了事件驅動异步IO非阻塞模型的各種優點,但是裡面有個關鍵詞叫"I/O",如果是非I/O的處理例如CPU計算還是沒改進的,如果有长时间运行的计算，将会导致CPU时间片不能释放，使得后续I/O无法发起.
可以通過把密集運算拆分成多個小任務,減輕CPU壓力.

3, 不能用到CPU的多核
現在的服务器操作系统基本都是支持多CPU/核了,單線程言語註定只能佔用一個資源,不能充分利用.
