...
[EventProxy英文](https://github.com/JacksonTian/eventproxy/blob/master/README_en.md)
...
##async.js
[async.js](https://caolan.github.io/async/)是一个强大实用的异步库,提供多达70+API,包括一些常用的功能函数 (map, reduce, filter, each…)和流程控制函数(parallel, series, waterfall…),所有方法都假定你遵循nodejs约定:
1, 提供一个回调函数作为最后入参;
2, 错误作为返回值第一个参数;
3, 只执行一次回调;
...
#異步并發控制
##Bagpipe
[Bagpipe](http://html5ify.com/bagpipe/#index_API)通过push将调用传入内部队列。如果活跃调用小于最大并发数，将会被取出直接执行，反之则继续呆在队列中。当一个异步调用结束的时候，会从队列前取出调用执行。以此来保证异步调用的活跃量不高于限定值。
