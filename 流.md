...
# 流（stream）
[color=#ec7379]流（stream）[/color]是一种可读写的在 Node.js 中处理流式数据的抽象接口，所有的流都是 [color=#ec7379]EventEmitter[/color] 的实例。

之所以出现这个模块是因为之前在[Nodejs内存控制](https://www.qdfuns.com/article/40831/8004b9968520abc963e2f954fffd5d7f.html)有提到过，Nodejs 提供了 Stream模块 用于处理大文件，它继承自 EventEmitter ，所以具备基本的自定义事件功能，同时抽象出标准的事件和方法， Nodejs 大多数模块都会用到 Stream模块，得益于它实现方式[bgcolor=#ffe9ec]不受V8内存限制，能够有效提高代码健壮性[/bgcolor]。
...
## 对象模式
所有 Node.js API 创建的流都是专门运作在[color=#ec7379]字符串[/color]和 [color=#ec7379]Buffer[/color]（或 [color=#ec7379]Uint8Array[/color]）对象上。 当然，流的实现也可以使用其它类型的 JavaScript 值（除了 [color=#ec7379]null[/color]，因为它在流中有特殊用途）。 这些流会以“对象模式”进行操作。

当创建流时，可以使用 [color=#ec7379]objectMode[/color] 选项把流实例切换到对象模式，[bgcolor=#ffe9ec]试图将已经存在的流切换到对象模式是不安全的[/bgcolor]。


## 缓冲
可写流和可读流都会在一个内部的缓冲器中存储数据，可以分别使用 [color=#ec7379]writable.writableBuffer 或 readable.readableBuffer[/color] 来获取。

可缓冲的数据的数量取决于传入流构造函数的 [color=#ec7379]highWaterMark[/color] 选项。 对于普通的流，highWaterMark 选项指定了[color=#ec7379]字节[/color]的总数量。 对于以对象模式运作的流，highWaterMark 指定了[color=#ec7379]对象[/color]的总数量。

当调用 [color=#ec7379]stream.push(chunk)[/color] 时，数据会被缓冲在可读流中。 如果流的消耗程序没有调用 [color=#ec7379]stream.read()[/color]，则这些数据会停留在内部队列中，直到被消耗。

一旦内部的可读缓冲的总大小达到 highWaterMark 指定的阈值时，流会暂时停止从底层资源读取数据，直到当前缓冲的数据被消耗 （也就是说，流会停止调用内部的用于填充可读缓冲的 readable._read() 方法）。

当反复地调用 [color=#ec7379]writable.write(chunk)[/color] 方法时，数据会被缓冲在可写流中。 当内部的可写缓冲的总大小小于 highWaterMark 设置的阈值时，调用 writable.write() 会返回 true。 一旦内部缓冲的大小达到或超过 highWaterMark 时，则会返回 false。

stream API 的主要目标，特别是 [color=#ec7379]stream.pipe()[/color] 方法，是为了将数据缓冲限制在可接受的级别，以便不同速度的源和目的地不会超过可用内存。

因为 [color=#ec7379]Duplex 和 Transform[/color] 都是可读又可写的，所以它们各自维护着两个相互独立的内部缓冲器用于读取和写入， 这使得它们在维护数据流时，读取和写入两边可以各自独立地运作。 例如，net.Socket 实例是 Duplex 流，它的可读端可以消耗从 socket 接收的数据，而可写端则可以将数据写入到 socket。 因为数据写入到 socket 的速度可能比接收数据的速度快或者慢，所以在读写两端独立地进行操作（或缓冲）就显得很重要了。


# 可写流（stream.Writable 类）
可写流是对数据要被写入的目的地的一种抽象，所有可写流都实现了 [color=#ec7379]stream.Writable类[/color] 定义的接口。

我们先以 [color=#ec7379]fs模块[/color] 的 [color=#ec7379]fs.WriteStream 类[/color] 为例运行一次看看效果，然后再针对stream.Writable 类的方法事件做进一步详解。
常规事件：
...
```
([color=#999]完整代码可以执行[stream-demo](https://github.com/Lin-Kyle/stream-demo)的 lesson1 查看效果[/color])
...
### 'drain' 事件
如果调用 [color=#ec7379]stream.write(chunk)[/color] 方法返回 false，则在适合恢复写入数据到流时触发 'drain' 事件。

### 'error' 事件
当写入数据出错或使用管道出错时，触发 'error' 事件。 监听器回调函数被调用时会传入一个 Error 参数，但[bgcolor=#ffe9ec]触发 'error' 事件时，流还未被关闭[/bgcolor]。
...
writable.write() 方法向流中写入数据，并在数据处理完成后调用 callback 。[bgcolor=#ffe9ec]如果有错误发生， callback 不一定以这个错误作为第一个参数并被调用，要确保可靠地检测到写入错误，应该监听 'error' 事件。[/bgcolor]
...
如果想根据需要生成或者取得将要被写入的数据，我们建议将逻辑封装为一个可读流并且使用 [color=#ec7379]stream.pipe()[/color]。 然而如果调用 write() 优先, 那么可以使用 'drain' 事件来防止回压并且避免内存问题:
...
```
([color=#999]完整代码可以执行[stream-demo](https://github.com/Lin-Kyle/stream-demo)的 lesson2 查看效果[/color])
...
调用 writable.end() 方法表明接下来没有数据要被写入可写流。通过传入可选的 chunk 和 encoding 参数，可以在关闭流之前再写入一段数据。如果传入了可选的 [bgcolor=#ffe9ec]callback 函数，它将作为 'finish' 事件的回调函数。[/bgcolor]
...
```
([color=#999]完整代码可以执行[stream-demo](https://github.com/Lin-Kyle/stream-demo)的 lesson3 查看效果[/color])
...
```
([color=#999]完整代码可以执行[stream-demo](https://github.com/Lin-Kyle/stream-demo)的 lesson4 查看效果[/color])


### writable.cork()
调用 writable.cork() 方法将强制所有写入数据都存放到内存中的缓冲区里。 直到调用 [color=#ec7379]stream.uncork() 或 stream.end()[/color] 方法时，缓冲区里的数据才会被输出。
...
如果使用 writable.cork() 和 writable.uncork() 来管理写入缓存，建议使用 [color=#ec7379]process.nextTick()[/color] 来延迟调用 writable.uncork() 方法。通过这种方式，可以对单个 Node.js 事件循环中调用的所有 writable.write() 方法进行批处理。

[bgcolor=#ffe9ec]如果一个流多次调用了 writable.cork() 方法，那么也必须调用同样次数的 writable.uncork() 方法以输出缓冲区数据。[/bgcolor]
```
const fs = require('fs');
...
```
([color=#999]完整代码可以执行[stream-demo](https://github.com/Lin-Kyle/stream-demo)的 lesson5 查看效果[/color])
...
//16384 0
```
([color=#999]完整代码可以执行[stream-demo](https://github.com/Lin-Kyle/stream-demo)的 lesson6 查看效果[/color])
...
可读流是对提供数据的来源的一种抽象，所有的可读流都实现了 [color=#ec7379]stream.Readable类[/color] 上定义的接口。
...
//ready
//Received 3807 bytes of data.
//There will be no more data.
//close
```
([color=#999]完整代码可以执行[stream-demo](https://github.com/Lin-Kyle/stream-demo)的 lesson8 查看效果[/color])

## 两种模式
可读流实质上运作于[color=#ec7379]流动中（flowing）[/color]或[color=#ec7379]已暂停（paused）[/color]两种模式之一。
...
需要记住的重要概念是，[bgcolor=#ffe9ec]只有提供了消耗或忽略数据的机制后，可读流才会产生数据。 如果消耗的机制被禁用或移除，则可读流会停止产生数据。[/bgcolor]
...
```
([color=#999]完整代码可以执行[stream-demo](https://github.com/Lin-Kyle/stream-demo)的 lesson7 查看效果[/color])
...
如果调用 [color=#ec7379]readable.setEncoding()[/color] 方法明确为流指定了默认编码，回调函数将接收到一个字符串，否则接收到的数据将是一个 Buffer 实例。
...
[bgcolor=#ffe9ec]注意： 'end' 事件只有在数据被完全消耗后才会触发 。 可以在数据被完全消耗后，通过将流转换到 flowing 模式， 或反复调用 stream.read() 方法来实现这一点。[/bgcolor]
```
const readable = getReadableStreamSomehow();
readable.on('data', (chunk) => {
  console.log(`Received ${chunk.length} bytes of data.`);
...
```
([color=#999]完整代码可以执行[stream-demo](https://github.com/Lin-Kyle/stream-demo)的 lesson9 查看效果[/color])
...
### readable.resume()
readable.resume() 方法会[bgcolor=#ffe9ec]重新触发data事件, 将暂停模式切换到流动模式[/bgcolor]，也可以用来充分使用流中的数据，而不用实际处理任何数据，如以下示例所示：
```
...
### readable.pause()
readable.pause() 方法将会使 flowing 模式的流[bgcolor=#ffe9ec]停止触发 'data' 事件， 进而切出 flowing 模式[/bgcolor]。任何可用的数据都将保存在内部缓存中。
```
const readable = getReadableStreamSomehow();
readable.on('data', (chunk) => {
    console.log(`Received ${chunk.length} bytes of data.`);
...
### readable.isPaused()
readable.isPaused() 方法返回可读流的当前操作状态。 该方法主要是在 [color=#ec7379]readable.pipe()[/color] 方法的底层机制中用到。大多数情况下，没有必要直接使用该方法。
```
...
readable.pipe() 绑定一个 [color=#ec7379][Writable][][/color] 到 readable 上， 将可写流自动切换到 flowing 模式并将所有数据传给绑定的 [Writable][]。数据流将被自动管理。这样，即使是可读流较快，目标可写流也不会[color=#ec7379]超负荷（overwhelmed）[/color]。
...
默认情况下，当[color=#ec7379]源可读流（the source Readable stream）[/color]触发 'end' 事件时，目标流也会调用 [color=#ec7379]stream.end()[/color] 方法从而结束写入。要禁用这一默认行为， end 选项应该指定为 false， 这将使目标流保持打开， 如下面例子所示：
...
注意：[bgcolor=#ffe9ec]不管对 process.stderr 和 process.stdout 指定什么选项，它们都是直到 Node.js 进程退出才关闭。[/bgcolor]
...
[bgcolor=#ffe9ec]readable.read()方法只应该在暂停模式下的可读流上运行。在流模式下，readable.read()自动调用直到内部缓冲区的数据完全耗尽。[/bgcolor]
...
[color=#b1b1b1]\(不太懂，附上原文)[/color]

注意: [bgcolor=#ffe9ec]'end' 事件已经触发或者运行时错误抛出后，stream.unshift(chunk) 方法不能被调用。[/bgcolor]

使用 stream.unshift() 的开发者一般需要换一下思路，考虑用一个[color=#ec7379][Transform][][/color] 流替代. 更多信息请查看[API for Stream Implementers](http://nodejs.cn/api/stream.html#stream_api_for_stream_implementers)部分。
...
```
注意： 不像 stream.push(chunk)，[bgcolor=#ffe9ec]stream.unshift(chunk)在重置流的内部读取状态时是不会结束读取过程。 如果在读取过程中调用 readable.unshift() 则会导致异常[/bgcolor] (例如：即来自自定义流上的 stream._read()内部方法上的实现)。 应该在调用 readable.unshift()方法之后适当调用 stream.push('') 来重置读取状态，执行读取的过程中最好避免调用 readable.unshift()方法。
...
```
[color=#b1b1b1]\(这还是一个新的特性，了解即可)[/color]
...
## stream.Duplex 类
Duplex 流是同时实现了 [color=#ec7379][Readable][] 和 [Writable][][/color] 接口的流。
...
## stream.Transform 类
[color=#ec7379]变换流（Transform streams）[/color] 是一种 [color=#ec7379][Duplex][][/color] 流。它的输出与输入是通过某种方式关联的。和所有 [Duplex][] 流一样，变换流同时实现了 [color=#ec7379][Readable][] 和 [Writable][][/color] 接口。
...
```
([color=#999]完整代码可以执行[stream-demo](https://github.com/Lin-Kyle/stream-demo)的 lesson1 查看效果[/color])1
在处理流的提前销毁（如被抛弃的HTTP请求）等错误事件时特别有用，[bgcolor=#ffe9ec]此时流不会触发 'end' 或 'finish' 事件[/bgcolor]。
...
/*流发生错误 { [Error: ENOENT: no such file or directory, open 'C:\project\test\stream-demo\archive.tar']
  ...
  path: 'C:\\project\\test\\stream-demo\\archive.tar' }*/
```
([color=#999]完整代码可以执行[stream-demo](https://github.com/Lin-Kyle/stream-demo)的 lesson1 查看效果[/color])2
...
在Node.js v0.10及更高版本中，套接字仍然存在永远停顿。在这种情况下的解决方法是调用[color=#ec7379]stream.resume()[/color]开始读取数据：
```
 // Workaround
net.createServer((socket) => {
...
首先，一个流开发者可能声明了一个JavaScript类并且继承四个基本流类中的一个（[color=#ec7379]stream.Writeable，stream.Readable，stream.Duplex，或者stream.Transform[/color]），确保他们调用合适的父类构造函数:
```
...
[color=#b1b1b1]（因为毕竟实际开发中很少需要自己去实现流的代码，所以我简单说说这章就算跳过了，有兴趣的自行研究吧）[/color]


##完整demo
[stream-demo](https://github.com/Lin-Kyle/stream-demo)
里面demo都是自己写的，没有依赖可以直接跑。懒得写就去上面搬走看，懒得搬就直接看文章，大部分代码连输出信息都给你们了。


## 参考资料
[Node.js v10.6.0 文档](http://nodejs.cn/api/stream.html#stream_api_for_stream_implementers)
[Node.js v10.7.0 Documentation](https://nodejs.org/docs/latest/api/stream.html#stream_readable_unshift_chunk)
